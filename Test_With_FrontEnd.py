...
st.set_page_config(page_title="INNControl Processor", layout="wide")
st.title("INNControl - Financial Automation")
        # === Inserted original processing logic below ===
...
def process_far_file(uploaded_file):
            # Read uploaded file once, use separate buffers for openpyxl and pandas
            uploaded_data = uploaded_file.read()
            file_for_openpyxl = BytesIO(uploaded_data)
            file_for_pandas = BytesIO(uploaded_data)

            # Always reset buffer before pandas read
            file_for_pandas.seek(0)
            df_far_head = pd.read_excel(file_for_pandas, sheet_name='FAR', header=None, nrows=5)
            ...
            # 2. Load workbook
            file_for_openpyxl.seek(0)
            wb = openpyxl.load_workbook(file_for_openpyxl)

            # 3. MODULE 1: Split Account Transactions into separate sheets
            ...
            # 4. MODULE 2: Process Mappings sheet and apply FormatN logic
            if 'Mappings' in wb.sheetnames:
                # Always reset buffer before pandas read
                file_for_pandas.seek(0)
                df_mappings = pd.read_excel(file_for_pandas, sheet_name='Mappings')
                mappings = []
                ...
                for itemName, functionName in mappings:
                    st.info(f"Processing mapping: Item '{itemName}' with Format '{functionName}'")
                    target_sheet = None
                    ...
                    if target_sheet is not None:
                        ...
                        st.info(f"Executing {fmt} for item '{itemName}' on sheet '{target_sheet.title}'")
                        # --- Format1 ---
                        if fmt == 'format1':
                            st.info(f"Format1 logic would run here for '{itemName}'")
                            target_sheet.cell(row=15, column=1).value = "Format1 summary table placeholder"
                            format_summary_table(target_sheet, start_row=15)
                        # --- Format2 ---
                        elif fmt == 'format2':
                            st.info(f"Format2 logic would run here for '{itemName}'")
                            target_sheet.cell(row=15, column=1).value = "Format2 summary table placeholder"
                            format_summary_table(target_sheet, start_row=15)
                        # --- Format3 ---
                        elif fmt == 'format3':
                            st.info(f"Format3 logic would run here for '{itemName}'")
                            target_sheet.cell(row=15, column=1).value = "Format3 summary table placeholder"
                            format_summary_table(target_sheet, start_row=15)
                        # --- Format4 ---
                        elif fmt == 'format4':
                            st.info(f"Format4 logic would run here for '{itemName}'")
                            target_sheet.cell(row=15, column=1).value = "Format4 summary table placeholder"
                            format_summary_table(target_sheet, start_row=15)
                        # --- Format5 ---
                        elif fmt == 'format5':
                            st.info(f"Format5 logic would run here for '{itemName}'")
                            target_sheet.cell(row=15, column=1).value = "Format5 summary table placeholder"
                            format_summary_table(target_sheet, start_row=15)
                        # --- Format6 ---
                        elif fmt == 'format6':
                            st.info(f"Format6 logic would run here for '{itemName}'")
                            target_sheet.cell(row=15, column=1).value = "Format6 summary table placeholder"
                            format_summary_table(target_sheet, start_row=15)
                        # --- Format7 ---
                        elif fmt == 'format7':
                            if "Account Transactions" in wb.sheetnames:
                                st.info(f"Running Format7 for '{itemName}' with Account Transactions sheet.")
                                wsTrans = wb["Account Transactions"]
                                # ...existing Format7 logic...
                            else:
                                st.warning("Account Transactions sheet not found for Format7")
                        # --- Format8 ---
                        elif fmt == 'format8':
                            if "Account Transactions" in wb.sheetnames:
                                st.info(f"Running Format8 for '{itemName}' with Account Transactions sheet.")
                                wsTrans = wb["Account Transactions"]
                                # ...existing Format8 logic...
                            else:
                                st.warning("Account Transactions sheet not found for Format8")
                        # --- Format9 ---
                        elif fmt == 'format9':
                            if "Account Transactions" in wb.sheetnames:
                                st.info(f"Running Format9 for '{itemName}' with Account Transactions sheet.")
                                wsTrans = wb["Account Transactions"]
                                # ...existing Format9 logic...
                            else:
                                st.warning("Account Transactions sheet not found for Format9")
                        # --- Format10 ---
                        elif fmt == 'format10':
                            if "Account Transactions" in wb.sheetnames:
                                st.info(f"Running Format10 for '{itemName}' with Account Transactions sheet.")
                                wsTrans = wb["Account Transactions"]
                                # ...existing Format10 logic...
                            else:
                                st.warning("Account Transactions sheet not found for Format10")

            # 5. Continue with FAR and other modules as before
            # ...existing code for FAR, depreciation, months, tables, etc...
                            ws = target_sheet  # Ensure ws is initialized
                            summaryRow = 15  # Ensure summaryRow is initialized
                            currentRow = 1  # Ensure currentRow is initialized
                            monthDict = {}  # Ensure monthDict is initialized
                            openingBalance = 0  # Ensure openingBalance is initialized
                            closingBalance = 0  # Ensure closingBalance is initialized
                            found = False  # Ensure found is initialized
                            accountRow = None  # Ensure accountRow is initialized
                            transactionDate = wsTrans.cell(row=currentRow, column=1).value if currentRow <= lastRow else None
            ...
            # Always read uploaded_file into a BytesIO buffer and use it for all reads
            if hasattr(uploaded_file, 'read'):
                file_bytes = uploaded_file.read()
                excel_buffer = BytesIO(file_bytes)
            else:
                excel_buffer = BytesIO(uploaded_file)
            ...
            if 'FAR' in wb.sheetnames:
                ...
                file_for_pandas.seek(0)
                df_raw = pd.read_excel(file_for_pandas, sheet_name='FAR', header=None)
            static_headers = [
                    'Purchase Date',
                    'Details',
                    'Cost',
            ...
            if 'Account Transactions' in wb.sheetnames:
                ...
                # Reset buffer pointer before reading again
                file_for_pandas.seek(0)
                df_trans_raw = pd.read_excel(file_for_pandas, sheet_name='Account Transactions', header=None)
            ...
            # ...existing code...
            # Output only to BytesIO for Streamlit download, no local file/folder output.
            ...
            with st.container():
                ...
                if not st.session_state['processing']:
                    ...
                    with st.spinner("Processing... Please wait."):
                        ...
                        try:
                            # Call the main processing function to get the processed workbook as BytesIO
                            output_stream = process_far_file(uploaded_file)
                            ...
                            st.download_button(
                                ...
                                file_name="Processed_INNControl.xlsx",
