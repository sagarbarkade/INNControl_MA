            ...
            if 'management accounts' in val.lower():
                ...
                if match3:
                    try:
                        ...
                        pass
    if not year_end_date:
        print(f"Could not extract year-end date from FAR sheet in {input_file}.")
        exit(1)
    if not period_end_date:
        print(f"Could not extract period end date (management account date) from FAR sheet in {input_file}.")
        exit(1)

    # 2. Set financial year start and end
    fy_end = pd.to_datetime(year_end_date)
    fy_start = fy_end - pd.DateOffset(years=1) + pd.DateOffset(days=1)
    mgmt_acct_month = pd.to_datetime(period_end_date)
